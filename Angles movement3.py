...
# calculating distance by Euclidean distance:
def distance(x1, y1, x2, y2):
    return math.sqrt((x2 - x1) ** 2 + (y2 - y1) ** 2)

def get_region_center(current_landmarks, region_indices):
    """Return center point of a body region."""
    points = []
...
def compute_region_motion(current_landmarks, prev_landmarks, region_indices):
    """Average motion of all landmarks in one region."""
    motions = []
    ...
    for idx in region_indices:
        if idx in current_landmarks and idx in prev_landmarks:
            ...
            motions.append(distance(px, py, cx, cy))
...
while True:
    ...
    if last_results and last_results.pose_landmarks:
        ...
        # -----------------------------
        # Compute motion for each region
        # -----------------------------
        region_motions = {}

        for region_name, region_indices in BODY_REGIONS.items():
            region_motion = compute_region_motion(
                current_landmarks,
                prev_landmarks,
                region_indices
            )
            region_motions[region_name] = region_motion

        # Find most moving region
        moving_region = None
        max_region_motion = 0

        for region_name, motion_value in region_motions.items():
        ...
        if (
            ...
            print(f"Detected: {display_region} | Motion: {display_motion:.2f}")

        # -----------------------------
        # Draw selected region
        # -----------------------------
        if display_region != "None":
            draw_text_bg(img, f"Moving: {display_region}", (10, 65), scale=0.6, thickness=2)
            draw_text_bg(img, f"Motion: {display_motion:.1f}", (10, 100), scale=0.6, thickness=2)

            if display_center is not None:
                cx, cy = display_center
                cv2.circle(img, (cx, cy), 12, (0, 0, 255), cv2.FILLED)
                draw_text_bg(img, display_region, (cx + 10, cy - 10), scale=0.5, thickness=1)
        else:
            draw_text_bg(img, "Moving: None", (10, 65), scale=0.6, thickness=2)
    ...
    cv2.imshow("Body Region Motion Detection", img)
